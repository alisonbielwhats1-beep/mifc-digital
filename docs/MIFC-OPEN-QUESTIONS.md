...
| `OQ31` | Produção + Qualidade | Um `RAIL_ID` distinto em cada saída equivale sempre a uma peça física boa? Como retrabalho, refugo e repetição são tratados? | a contagem técnica fechou, mas a unidade física ainda não está aprovada | snapshot `OMES-2026-08-20T1137-BRT` e comentários DAX “Produção Peças” |
| `OQ32` | PCP + BI + Produção | Como conciliar produção e demanda pelo mesmo cliente/pedido quando há produção com demanda diária zero ou cliente diferente da família de demanda? | restante negativo e utilização podem ser interpretados incorretamente | Beatty 2/3 e CNC com demanda 0; Beatty 2/4 produziram VDB no snapshot |
