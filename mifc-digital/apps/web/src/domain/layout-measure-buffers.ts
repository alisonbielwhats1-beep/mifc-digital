...
interface MeasureBufferDefinition {
  ...
  offsetX?: number;
  offsetY?: number;
  measures: Array<[LayoutClientKey | "ALL", string, string?]>;
}

const clientLabels: Record<LayoutClientKey | "ALL", string> = {
  FH: "FH",
  VM: "VM",
  SCA: "Scania",
  DAF: "DAF",
...
export function buildLayoutMeasureBuffers(
  ...
  return definitions.flatMap((definition) => {
    ...
    return [{
      ...
      values: definition.measures.map(([clientKey, measureKey, label]) => ({
        clientKey,
        clientLabel: label ?? clientLabels[clientKey],
        measureKey,
