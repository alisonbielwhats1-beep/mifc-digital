...
async function save() {
  const invalid = logisticsRows.value.some((row) => !row.customer.trim() || !row.plannedDate || !row.plannedTime || row.transportHours < 0 || row.movementMinutes < 0 || row.shipmentLotSize < 0);
...
<template>
  <div class="page">
    ...
    <section class="form-layout">
        ...
        <article class="card table-shell">
          ...
          <div class="table-scroll">
            <table class="data-table logistics-table">
              <thead><tr class="column-band"><th colspan="3"></th><th colspan="7">Input (editável)</th><th colspan="2"></th></tr><tr><th>Cliente</th><th>Veículo</th><th>Flatbed</th><th>Data programada</th><th>Horário</th><th>Transporte (h)</th><th>Movimentação (min)</th><th>Frequência</th><th>Lote</th><th>Status</th><th>Origem</th><th><span class="sr-only">Ações</span></th></tr></thead>
              <tbody>
                <tr v-for="row in logisticsRows" :key="row.id" :class="{ 'inactive-row': row.status === 'inactive' }">
                  ...
                  <td><input v-model="row.plannedDate" class="table-input" type="date" aria-label="Data programada" /></td><td><input v-model="row.plannedTime" class="table-input" type="time" aria-label="Horário programado" /></td><td><input v-model.number="row.transportHours" class="table-input number-input" type="number" min="0" step="0.25" aria-label="Horas de transporte" /></td><td><input v-model.number="row.movementMinutes" class="table-input number-input" type="number" min="0" step="1" aria-label="Minutos de movimentação" /></td><td><input v-model.trim="row.shipmentFrequency" class="table-input" aria-label="Frequência de embarque" /></td><td><input v-model.number="row.shipmentLotSize" class="table-input number-input" type="number" min="0" step="1" aria-label="Tamanho do lote" /></td>
      ...
      <FormSourcePanel><strong class="source-title"><Route :size="15" />Cobertura do MIPS</strong><p>Programação, data, horário, flatbed, transporte, frequência e lote estão disponíveis. Material, item, localização, ship date e quantidades aguardam a conexão Oracle somente leitura.</p></FormSourcePanel>
    </section>
  </div>
</template>
...
.logistics-table { min-width: 1450px; }
