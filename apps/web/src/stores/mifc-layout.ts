...
function initialRevision(): LayoutRevision {
  ...
  const nodes = [
    ...
    node(r,"node-weld-1","process","Mesa 3",680,455,136,90,properties("P-003",0,0,0,2,90,"T-M3 é placeholder zero no PBIP; validar operacionalmente.","T-M3")),
    node(r,"node-beatty-3","process","Beatty 3",850,300,132,88,properties("P-002-B3",62,132,928,2,82,"Scania conforme o PBIP.","T-B3"),"cap-beatty-3"),
    node(r,"node-beatty-4","process","Beatty 4",890,400,132,88,properties("P-002-B4",62,132,928,2,82,"Volvo FH conforme o PBIP.","T-B4"),"cap-beatty-4"),
    node(r,"node-beatty-2","process","Beatty 2",930,500,132,88,properties("P-002-B2",62,132,928,2,82,"DAF conforme o PBIP.","T-B2"),"cap-beatty-2"),
    node(r,"node-weld-2","process","Beatty 1",970,600,132,88,properties("P-002-B1",62,132,928,2,82,"Volvo VM conforme o PBIP.","T-B1"),"cap-beatty"),
    node(r,"node-weld-3","process","P.A / CNC",1160,455,136,90,properties("P-006/007",0,0,0,2,90,"P.A para FH/Scania; CNC para VM/DAF.","T-P.A/T-CNC")),
    node(r,"node-assembly","process","Pintura / Rebitagem",1340,455,144,90,properties("P-003",110,150,528,2,60,"Pintura comum; rebitagem adicional para Scania e DAF.","T-LPP2"),"cap-paint"),
    node(r,"node-inspection","process","Stenhoj / Embalagem",1530,455,144,90,properties("P-004",60,110,960,2,90,"VM termina em embalagem; demais clientes usam Stenhoj e embalagem.","T-STJ/T-EMB-VM"),"cap-stenhoj"),
    node(r,"node-finished","storage","Armazém\nProduto acabado",1720,454,126,94,properties("BUF-002",0,100,720,2,95,"","D-E-PA")),
    node(r,"node-shipping","truck","Expedição",1880,462,100,78,properties("LOG-001")),
    node(r,"node-volvo","customer_supplier","VOLVO",2070,350,96,64,properties("CLI-001")),
    node(r,"node-scania","customer_supplier","SCANIA",2070,438,96,64,properties("CLI-002")),
    node(r,"node-daf","customer_supplier","DAF",2070,526,96,64,properties("CLI-003")),
    node(r,"node-renault","customer_supplier","RENAULT",2070,614,96,64,properties("CLI-004")),
    node(r,"node-toolroom","information","Ferramentaria",450,700,150,66,properties("SUP-001")),
    node(r,"node-maintenance","information","Manutenção",640,700,150,66,properties("SUP-002")),
    node(r,"node-lab","information","Laboratório",830,700,150,66,properties("SUP-003")),
    node(r,"node-quality-control","information","Controle de\nQualidade",1020,700,160,66,properties("SUP-004")),
  ];
...
export const useMifcLayoutStore = defineStore("mifc-layout", {
  ...
  actions: {
    ...
    moveNode(id: string, x: number, y: number) { const item = this.activeRevision.nodes.find((entry) => entry.id === id); if (item) { item.x = clamp(x, 0, LAYOUT_WORLD_WIDTH - item.width); item.y = clamp(y, 0, 790 - item.height); } },
